...
from forth.groundtruth_move import GroundTruth
from forth.groundtruth_move39 import GroundTruth as GroundTruth39
from forth.groundtruth_move60 import GroundTruth as GroundTruth60
...
class MultiagentInformationGathering:
	...
	def __init__(self,
		...
		if ground_truth_type == '50':
			self.gt=GroundTruth(self.scenario_map)
		if ground_truth_type == '39':
			self.gt=GroundTruth39(self.scenario_map)
		if ground_truth_type == '60':
			self.gt=GroundTruth60(self.scenario_map)
		...
		self.observation_space = gym.spaces.Box(low=0.0, high=1.0, shape=(6, *self.scenario_map.shape), dtype=np.float32)

		self.state_space = gym.spaces.Box(low=0.0, high=1.0, shape=(6, *self.scenario_map.shape), dtype=np.float32)
		#这里为什么是5 也不清楚
		...
		print("gp_positions",gp_positions.shape)
    ...
    #    [31, 31]])
		...
		if self.local:
			self.gp_coordinator = LocalGaussianProcessCoordinator(gp_positions = gp_positions,
								# kernel = C(1.0)*RBF(length_scale=10, length_scale_bounds=(8, 12)) + W(noise_level=1e-5, noise_level_bounds=(1e-5, 1e-5)),
								
        						kernel = C(1.0)*RBF(length_scale=5.0, length_scale_bounds=(0.5, 10.0)) + W(noise_level=1e-5, noise_level_bounds=(1e-5, 1e-1)),#-5
	...
	def step(self, actions):
		...
		# print(self.measurements[0][0])
# 		[[0.03923863]
#  [0.0371735 ]
#  [0.31550819]]
		# input()
		#采取一系列动作后用户位置改变：
#如果每一个step，无人机移动的时候，users也移动，效果很差
# 尝试每一个epo，用户移动一次，即在无人机移动的时候用户不动
		self.gt.step()
	...
	def compute_reward(self, collisions):
		...
		# print("changes_mu_values",len(changes_mu_values))
		# input()
  		# changes_mu_values= self.gp_coordinator.get_local_gp_changes()
		
  		# changes_sigma_values =0
		# get_local_gp_changes
		...
		# print("agent_changes",agent_changes)
		...
		if self.re=='IG':
			if self.local:
				...
				reward = {agent_id: agent_changes[agent_id] for agent_id in range(self.number_of_agents)}
		elif self.re=='SG':
			reward = {agent_id: self.gt.read(self.fleet.agent_positions)[agent_id] for agent_id in range(self.number_of_agents)}
		else:
			reward = {agent_id: 0.5*(agent_changes[agent_id] - penalization[agent_id])+(self.gt.read(self.fleet.agent_positions)[agent_id]) for agent_id in range(self.number_of_agents)}
		# 0.1 太小了显然
		# 换成 0.5
