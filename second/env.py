...
from second.gatherSignalInformation import gatherSignalInformation
from second.local_GPV1 import LocalGaussianDistribution


class env:
    
    def __init__(self,users,start_pos):
        
        super(env, self).__init__()
        self.local_gaussian = LocalGaussianDistribution()
        self.start_pos=start_pos
        self.state=np.array(start_pos).astype(np.float32)
        ...
        # *****
        # 这里从环境中不能直接获取user位置，而是得到一个signal
        
        # self.signal=gatherSignalInformation(self.state)
        # self.users=users#[(40,3),(44,0),(42,1)]#先找到单用户的最佳情况
        # *****
    ...
    def reset(self): # 获取state
        ...
        self.state=np.array(self.start_pos).astype(np.float32)
    ...
    def step(self,action):
        ...
        self.measurements=gatherSignalInformation(self.state)
        
        # self.gp_local.update(self.measurements)
        self.local_gaussian.add_point(self.state[0],self.state[1],self.state[2],self.measurements)
        # add point 后会同时update 
        # self.local_gaussian.get_distribution_parameters() 
