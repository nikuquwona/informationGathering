...
import json
import os
from thrid.ActionMaskingUtils import ConsensusSafeActionMasking, NoGoBackMasking, SafeActionMasking
from thrid.Networks.network import DistributionalVisualNetwork, DuelingVisualNetwork, NoisyDuelingVisualNetwork

from thrid.ReplayBuffers.ReplayBuffers import PrioritizedReplayBuffer
...
class MultiAgentDuelingDQNAgent:
	...
	def train(self, episodes):
		...
		if self.writer is None:
			...
			assert not os.path.exists(self.logdir), "目录已经存在。我阻止了它被覆盖"
		...
		for episode in trange(1, int(episodes) + 1):
			...
			while not all(done.values()):
				...
				# print("****************")
				# print("state",self.env.fleet.get_positions())
				...
				poss=[]
				for pos in self.env.fleet.get_positions():
					poss.append(pos)
				
				logging.info(f"state: {poss}")
