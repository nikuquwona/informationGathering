...
s=f'''# 十轮小步优化：未知聚类环境中的 UAV 部署\n\n## 本轮结论

验证集选出的第 {selected} 轮，在标准未见场景上为 **{stats(selected,"standard","trained","horizon_throughput_bps",1e6)[0]:.3f} Mbps**，低于原地不动的 **{stats(selected,"standard","stationary","horizon_throughput_bps",1e6)[0]:.3f} Mbps**；在细长地图上为 **{stats(selected,"elongated","trained","horizon_throughput_bps",1e6)[0]:.3f} Mbps**，也低于原地不动的 **{stats(selected,"elongated","stationary","horizon_throughput_bps",1e6)[0]:.3f} Mbps**。相对第 1 轮，标准场景略降、细长地图基本持平。
